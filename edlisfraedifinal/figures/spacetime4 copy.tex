...
\begin{tikzpicture}[scale=1.8]
  ...
  \coordinate (A) at (1/2.8,3/2.8); % observer A at t=0
  \coordinate (B) at (4/2.8,2/2.8); % observer B at t=0
  
  \fill[myblue] (A) circle(0.04) node[below right=-1] {A};
  \fill[mydarkgreen] (B) circle(0.04)
    node[right=4] {\contour{white}{$B$}};
...
\begin{tikzpicture}[scale=1.8]
  ...
  \foreach \i [evaluate={\x=\i*\d;}] in {1,...,\Nlines}{
    \message{  Running i/N=\i/\Nlines, x=\x...^^J}
    \draw[world line]   ( \x,-0.25) -- ( \x,\xmax);
    \draw[world line t] (-0.25, \x) -- (\xmax, \x);
  }
...
\begin{tikzpicture}[scale=1.8]
  ...
  \def\xmaxp{2.2} % maximum of rotated axis
  \def\Nlines{5} % number of world lines (at constant x/t)
  \pgfmathsetmacro\ang{atan(0.3)} % angle between x and x' axes
  \pgfmathsetmacro\d{0.9*\xmax/\Nlines} % grid size
  \pgfmathsetmacro\D{\d/cos(\ang)/sqrt(1-tan(\ang)^2)} % grid size, boosted
  \coordinate (O) at (0,0);
  \coordinate (X) at (\xmax+0.2,0);
  ...
  \coordinate (A) at (0.25,0.5); % observer A at t=0
  \coordinate (B) at (0.75,1.5); % observer B at t=0
  
  \fill[myblue] (A) circle(0.04) node[below right=-1] {A};
  \fill[mydarkgreen] (B) circle(0.04)
    node[right=4] {\contour{white}{$B$}};
  \fill[mydarkgreen] (0.75,1.5) circle(0.04)
    node[right=4] {\contour{white}{$B$}};
